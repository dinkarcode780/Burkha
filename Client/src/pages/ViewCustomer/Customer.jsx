...
//   if (filterType === "all") {
//     return item.type === "vendor" || item.type === "casula";
//   }
...
const handleSearch = debounce((searchValue) => {
  ...
  // pehle search filter lagao
  let result = state.orders.filter((order) => {
    return (
      order._id?.toLowerCase().includes(searchLower) ||
      order.orderItems[0]?.discountName?.firmName?.toLowerCase().includes(searchLower) ||
      order.orderItems[0]?.discountName?.mobile1?.toLowerCase().includes(searchLower) ||
      new Date(order.createdAt).toLocaleDateString().toLowerCase().includes(searchLower) ||
      `${order.totalPriceAfterDiscount ?? order.totalPrice}`.toLowerCase().includes(searchLower) ||
      order.status?.toLowerCase().includes(searchLower) ||
      order.paymentStatus?.toLowerCase().includes(searchLower)
  ...
  // abhi filterType ke hisaab se aur narrow kar do
  if (filterType === "casula") {
    result = result.filter(order => (order.orderItems[0]?.discountPercentage ?? 0) === 0);
  } else if (filterType === "vendor") {
    result = result.filter(order => (order.orderItems[0]?.discountPercentage ?? 0) === 1);
  } 
  // agar "all" hai toh result jaisa ka taisa rehne do
...
//     ? state.orders 
//     : state.orders.filter(order => order.type?.toLowerCase() === value.toLowerCase());
...
const handleFilterChange = (e) => {
  ...
  if (value === "casula") {
    filtered = state.orders.filter(order => (order.orderItems[0]?.discountPercentage ?? 0) === 0);
  } else if (value === "vendor") {
    filtered = state.orders.filter(order => (order.orderItems[0]?.discountPercentage ?? 0) === 1);
  } 
  // agar "all" hai toh filtered = state.orders hi rahega
  ...
  const columns = [
    ...
    name: "Mobile No",
    selector: (row) =>
      row.orderItems[0]?.discountName?.mobile1 ||
      row.mobile1 ||
      row.userPhone ||
      "N/A",
    sortable: true,
  ...
  name: "Type",
  selector: (row) => {
    const discount = row.orderItems[0]?.discountPercentage ?? 0;
    if (discount === 0) return "Casual";
    if (discount === 1) return "Vendor";
    return "N/A"; // agar aur kuch condition ho future me
  },
  sortable: true,
  width: "120px",
},
  ...
  cell: (row) => {
    const discount = row.orderItems[0]?.discountPercentage ?? 0;
    const buttonText = discount === 1 ? "Vendor Details" : "Casula Details";
    return (
      <button
        onClick={() => navigateToPayments(row._id)}
        className="px-3 py-1 bg-green-50 text-green-600 text-xs rounded hover:bg-green-100"
...
// mx-auto
  return (
    <div className="max-w-5xl py-8 ml-[12%]">
