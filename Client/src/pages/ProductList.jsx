...
const handleImport = (event) => {
  ...
  return (
    <div className='bg bg-white shadow rounded-lg max-w-5xl py-8 ml-[10%]'>
      {/* PRINT DIALOG */}
      {showPrintDialog && (
...
{previewImage && (
  ...
  >
    <div
      className="relative max-w-4xl w-full p-4 bg-white rounded-lg shadow-lg"
      onClick={(e) => e.stopPropagation()}
    >
      <button
        className="absolute top-4 right-4 text-black text-2xl font-bold bg-gray-200 rounded-full w-8 h-8 flex items-center justify-center hover:bg-gray-300 transition"
        onClick={() => setPreviewImage(null)}
      >
        &times;
      </button>
