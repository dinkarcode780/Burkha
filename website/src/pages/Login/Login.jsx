...
const Login = () => {
  ...
  const [formData, setFormData] = useState({ identifier: '', password: '', loginType: 'email' });
  const [errors, setErrors] = useState({});
  const [loginError, setLoginError] = useState('');
  ...
  const handleLoginTypeChange = (type) => {
    setFormData((prev) => ({ ...prev, loginType: type, identifier: '' }));
    setErrors({});
    setLoginError('');
  };
  ...
  const validate = () => {
    ...
    if (!formData.identifier) {
      newErrors.identifier = formData.loginType === 'email' ? 'Email is required' : 'Phone number is required';
    } else if (formData.loginType === 'email' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.identifier)) {
      newErrors.identifier = 'Invalid email format';
    } else if (formData.loginType === 'phone' && !/^\+?\d{10,15}$/.test(formData.identifier)) {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = formData.loginType === 'email' 
      ? { email: formData.identifier, password: formData.password }
      : { mobile1: formData.identifier, password: formData.password };
                  ...
                  <Card.Body className="p-4 p-sm-5">
                    ...
                    <Form onSubmit={handleSubmit}>
                      ...
                      <FloatingLabel
                        ...
                        label={formData.loginType === 'email' ? 'Email address' : 'Phone number'}
                        className="mb-24 mt-24"
                      >
                        <Form.Control
                          type={formData.loginType === 'email' ? 'email' : 'tel'}
                          name="identifier"
                          placeholder={formData.loginType === 'email' ? 'name@example.com' : '+1234567890'}
