...
const getPaymentsByOrderId = async (req, res) => {
  ...
  try {
   ...
   .populate({
        ...
        select: "phone mobile1 email firmName contactName", // Ensure phone or mobile1 is included
      })
      .populate({
        path: "orderId", // Populate the orderId to get order details
        select: "userPhone", // If order has userPhone
      })
      .sort({ createdAt: -1 });
      console.log("phone",payments)
    res.status(200).json({
      success: true,  
      payments,
    });
